...

{
		...
		: node_handle_(nh), arm_comm_(arm_comm), ik_solver_(ik_solver)
	{
        ...
        demoJaco = MatrixXf::Zero(5,6);
        mPJaco = MatrixXf::Zero(6,6);
        nPJaco = MatrixXf::Zero(6,3);
        demoPJaco = MatrixXf::Zero(6,5);
...
//		mPJaco = calcSingularityAwarePseudoJacobian(mJaco, 6);
	...
	void JacoKinematicController::calldemo(CartesianInfo dV)
	{
		mode = 4;
		...
		demoGoal(1) = dV.Y;
		demoGoal(2) = dV.Z;
		demoGoal(3) = dV.ThetaX;
		demoGoal(5) = dV.ThetaZ;

		updateConfig(mode);

		demoPJaco = calcDemoPJacobian(demoJaco);
//		mPJaco = calcSingularityAwarePseudoJacobian(mJaco, 6);

		dQ = calcdQ(demoPJaco, demoGoal, mode);
...
//            ROS_INFO("x: %f Y: %f", ee_pos.position.x, ee_pos.position.y);
	...
	MatrixXf JacoKinematicController::calcDemoPJacobian(MatrixXf mJ)
	{			
		MatrixXf mPJ1 = MatrixXf::Zero(5,5);
		MatrixXf mPJ2 = MatrixXf::Zero(6,5);
