...
void scanCallback (const sensor_msgs::LaserScan scan_msg)
{
  ...
  for (x = 0; x <= num_ranges; x=x+RangeDivider){
	...
	*/
    ...
    double minPeak=10;
    double indexPeak;
    for (x = 4; x <= (num_ranges-3); x=x+RangeDivider){
    	double RangeValue = scan_msg.ranges[x];
    	if(RangeValue>0.9 && RangeValue<1.4){
    		if(RangeValue<minPeak){
    		 	minPeak = RangeValue;
    		 	indexPeak = (double)x;
    		 }
		} 
    ...
    if(minPeak<10){
    	w = -(320.0-indexPeak)/320.0;
    	KeepGoing = true;
    	std::cout << "w: " << w << " minPeak: " << minPeak << " indexPeak: " << indexPeak << std::endl;
    } else {
    	w = 0;
		KeepGoing = false;
    }
    ...
    if(rangeMiddleValue>0 && rangeMiddleValue < 0.6){
    	KeepGoing = false;
    }
    */
