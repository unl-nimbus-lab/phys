    ...
    noise_dist_(0,1) //Gaussian noise. Normalized
{
    ...
    ground_truth_sub_ = nh_.subscribe("ground_truth", 100, &AtOdomSimNode::groundTruthCallback, this);
...
double AtOdomSimNode::getRate() const
{
    return rate_;
}

// void AtOdomSimNode::process()
// {
//     //to do 
// }
// 

void AtOdomSimNode::publish()
{
    ...
    if (odom_flag_) 
    {
        ...
        odom_msg.child_frame_id = "at_footprint";
        ...
        tfb_.sendTransform( tf::StampedTransform(odom2base, last_callback_stamp_, "odom", "at_footprint") );  
    }
}

void AtOdomSimNode::groundTruthCallback(const nav_msgs::Odometry::ConstPtr& _msg)
{
    //aux values
