...
void Action::poseHandInitWaypoints(std::vector<geometry_msgs::Pose> waypoints)
{
    ...
    waypoints.insert(waypoints.begin(), move_group_->getCurrentPose().pose);
    int i = 0;
    while(i < waypoints.size())
    {
        ROS_INFO_STREAM("Waypoint " << i << " is in pose:\n" << waypoints.at(i));
        i++;
    }

    double fraction = 0;
    int count = 0;
    int max_attempts_cart_path = 10;
    double min_fraction = 0.75;
    while(fraction < min_fraction && count < max_attempts_cart_path)
    {
