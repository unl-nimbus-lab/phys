...
public:
  ...
  void histogram(int d_id, int* o_id, int* o_conf, int* okaoId, 
		 double *maxHist, double *magni, long long tracking_id)
  {
    ...
    //一位と二位
    okaoId[0] = hist_to_okao[hist_pool[0]];
    okaoId[1] = hist_to_okao[hist_pool[1]];
    okaoId[2] = hist_to_okao[hist_pool[2]];
    *maxHist = hist_pool[0];
    *magni = (double)hist_pool[0]/(double)hist_pool[1];

    //cout << "okaoId[0]:"<<*okaoId[0]<<endl;
    //アウトプット用関数
...
*/
		   ...
		   std_msgs::Header src_header)
  {
	...
	if( src.face.persons.size() )
	  {
	    cout <<  src.face.persons.size() << endl;
	    for(int i = 0; i < OKAO; ++i)
	      {
		o_id[i] = src.face.persons[i].okao_id;
		o_conf[i] = src.face.persons[i].conf;
	      }
	    //personの保持
	...
	histogram( (d_id) , o_id, o_conf, okaoId, 
		   &maxHist, &magni, tracking_id );
	//personRecogProcess( tracking_id, &okaoId, maxHist,  magni );

	cout <<"face found[ " << ros::Time::now() 
	     << " ], d_id: "<<d_id << ", tracking_id: "
	     << tracking_id << " ---> max id: "
	     << okaoId[0] <<", max hist: " << maxHist << ", magni: " << magni << endl;
	...
	h.d_id = d_id;
	h.max_okao_id = okaoId[0];
	h.max_hist = maxHist;
	h.magni = magni;
	h.header.stamp = t;
	h.header.frame_id = src_header.frame_id;
	h.p = src.body.joints[ HEAD ].position;
	h.face.persons.push_back( prop_buf[ okaoId[0] ] );
	//h.face.persons.push_back( prop_buf[ okaoId[1] ] );
	//h.face.persons.push_back( prop_buf[ okaoId[2] ] );
