...
class WIDL
{
	...
	WIDL(int nbrIt_ = 1, int rate_ = 100, int scan_queue_size_ = 2 ) : rate(rate), scan_queue_size(scan_queue_size_)
	{
		...
		sigma2L = pow( 0.002f, 2);	//(5mm)^2 in paper
		...
		//sigma2Theta = pow(beta,2);	//odd value with that...
		//--------------------------------------------------------
		//--------------------------------------------------------
		//EKF :
		nbrstate = 6;
...
//TODO : estimate if an interval of points could be interesting...
		...
		for(int it=nbrIt;it--;)
		{
			...
			*/
			...
			for(int k=matches[0].getColumn();k--;)
			{
				...
				{
					//nbrpointsused++;
					residual = (pointsCartesianPose[0][k] - R*pointsCartesianPose[1][matches[0].get(1,k+1)-1]);
	...
	void computeEstimationCovariance()
	{
		float r = 0.0f;
