...
#define 	maxNBRPOINT 	500			//the incapacity to run at least at the same frame rate as the one of the sensor make the estimation flawed...
...
class WIDL
{
	...
	WIDL(int nbrIt_ = 1, int rate_ = 100, int scan_queue_size_ = 2 ) : rate(rate), scan_queue_size(scan_queue_size_)
	{
		...
		sigma2L = pow( 0.02f, 2);	//(5mm)^2 in paper
	...
	void computeDistanceMatrix()
	{
		...
		//TODO TRESH :
		float PolarTresh = PI/4;
		float RangeTresh = 0.6f;
...
#define doubleMatchesOnly	//more robust..
	...
	void estimateDeltaPose(float theta_)
	{
		...
		//initial guess of theta is to initialize the program :
		//------------------------------------	
		//------------------------------------
		//we have to move all the points of the previous  scan and rematch them again... :
		//TODO : debug and estimate if it is really necessary...
		/*
		for(int k=nbrPoints[1];k--;)
		{
			//let's move them : only in rotation
		...
		*/
		for(int it=nbrIt;it--;)
		{
			...
			*/
			...
			/*
			Mat<float> updatePose(2,1);
			updatePose.set( cos(theta)*dP.get(1,1) + sin(theta)*dP.get(2,1), 1,1);
			updatePose.set( -sin(theta)*dP.get(1,1) + cos(theta)*dP.get(2,1), 2,1);
		
			deltaP += updatePose;
			*/
			//it is already given in the current frame of the sensor since we moved the points at the end of each iterations...
			deltaP += dP;
			...
			//------------------------------------	
			//------------------------------------
			if(true || it != 0)	// indeed it has to be done at least for the estimation of theta..
			{
				//we have to move all the points of the previous  scan and rematch them again... :
				//TODO : debug...
#ifdef optimDP
...
#ifndef		iterativeHAYAI
			for(int iterationTheta = 2;iterationTheta--;)
			{		
				num =0.0f;
				denom = 0.0f;
