...
Actions::Actions(ros::NodeHandle nh)
{
  ...
  n_private.param("ang_vel_lim", ang_vel_lim_, 0.536);
...
// Changes
int Actions::GetCurrentActionFromOdom(nav_msgs::Odometry odom_msg)
{
  double ang_inc = 2 * ang_vel_lim_ / num_actions_;
  double angle = ang_vels_[0];
  int i = 0;
  
  while (i < num_actions_ - 1)
  {
    angle += ang_inc;
    if (odom_msg.twist.twist.angular.z < angle)
      break;
    i++;
  }
  
  ROS_INFO("Ang: %f, action: %d", odom_msg.twist.twist.angular.z, i);
  return i;
}
