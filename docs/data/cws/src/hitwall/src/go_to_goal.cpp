...
float x_initial[]={-6.0,-1.0,4.0,1.0,-5.0};
float y_initial[]={-4.0,-6.0,-6.0,6.0,5.0};
...
int main(int argc, char **argv)
{
	...
	char file_1[250];
	sprintf(file_1,"go_to_goal_robot_2_%i.csv",InitialCondition);
	char file_2[250];
	sprintf(file_2,"go_to_goal_polar_2_%i.csv",InitialCondition);
	...
	ros::Publisher pubName = goToGoalNode.advertise<geometry_msgs::Twist>("/mobile_base/commands/velocity", 1000);
	...
	while (ros::ok())
  	{
  		...
  		if(tsim>0){
			...
			a=minAngle(atan2(-ypos,-xpos)-theta);
			b=minAngle(-theta-a);
